        ...
        public Tokenizer<TokenTypes> Tokenizer
        {
            get
            {
                if (this.m_Tokenizer == null)
                {
                    ...
                    string directives = String.Join('|', new string[] { "define", "import", "using", "ifdef", "ifndef", "else", "endif" });

                    this.m_Tokenizer = new Tokenizer<TokenTypes>()
                    {
                        TokenDefinitions = new List<TokenDefinition<TokenTypes>>()
                        {
                            new TokenDefinition<TokenTypes>(TokenTypes.PreprocessorDirective, new Regex(@"^({directives})\s\w[a-z0-9]*"), 99),
